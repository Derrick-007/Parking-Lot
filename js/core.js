...
class Entry{
    constructor(owner,car,numberPlate,entryDate,exitDate){
        this.owner = owner;
        this.car = car;
...
class UI{
    ...
    static validateInputs(){
        ...
        // var numberPlateRegex = /^(?:[A-Z]{2}-\d{2}-\d{2})|(?:\d{2}-[A-Z]{2}-\d{2})|(?:\d{2}-\d{2}-[A-Z]{2})$/;
        if(owner === '' || car === '' || numberPlate === '' || entryDate === '' || exitDate === ''){
...
class Store{
    ...
    static removeEntries(numberPlate){
        const entries = Store.getEntries();
...
//Event Add
    document.querySelector('#entryForm').addEventListener('submit',(e)=>{
        ...
        //Instatiate Entry
        const entry = new Entry(owner, car, numberPlate, entryDate, exitDate);
...
//Event Remove
    document.querySelector('#tableBody').addEventListener('click',(e)=>{
        ...
        //Get nnumber plate to use as unique element of an entry
        var numberPlate = e.target.parentElement.previousElementSibling.previousElementSibling.previousElementSibling.textContent;
        //Call to Store function to remove entry from the local storage
        Store.removeEntries(numberPlate);
